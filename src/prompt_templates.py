def make_ctx(docs, per_chunk_chars=400):
    """Format a list of LangChain Document objects into numbered context snippets."""
    chunks = []
    for i, doc in enumerate(docs, 1):
        text = getattr(doc, "page_content", str(doc))  # Use .page_content if available
        text = text.strip().replace("\n", " ")
        chunks.append(f"[{i}] {text[:per_chunk_chars]}")
    return "\n\n".join(chunks)
